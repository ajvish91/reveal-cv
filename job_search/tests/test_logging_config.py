...
class LoggingConfigTests(unittest.TestCase):
    def test_configure_logging_writes_to_file(self) -> None:
        with self.subTest("file handler"):
            log_path = Path(self._testMethodName) / "job_search.log"
    ...
    def test_log_json_summary_does_not_crash(self) -> None:
        log_path = Path(self._testMethodName) / "summary.log"
        log_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_tail_log_file_returns_last_lines(self) -> None:
        log_path = Path(self._testMethodName) / "tail.log"
        log_path.parent.mkdir(parents=True, exist_ok=True)
